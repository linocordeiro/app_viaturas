...
def get_brasao_image(width=50, height=60):
    brasao_path = os.path.join(settings.BASE_DIR, 'static', 'img', 'brasao_pf.png')
    if os.path.exists(brasao_path):
        return Image(brasao_path, width=width, height=height)
    return None


def cabecalho_institucional(titulo_documento, subtitulo="SISTEMA DE CONTROLE DE ENTRADA E SAÍDA DE VIATURAS"):
...
def gerar_pdf_ficha(ficha):
    ...
    vigilante_txt += f"<br/>___________________________<br/>Assinatura"
...
def gerar_pdf_viaturas(viaturas):
    """
...
def gerar_pdf_manutencoes_viatura(viatura, manutencoes):
    """
