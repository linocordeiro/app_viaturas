...
class RegistroUso(models.Model):
    ...
    def clean(self):
        ...
        if self.odometro_chegada is not None and self.odometro_saida is not None:
            if self.odometro_chegada < self.odometro_saida:
                raise ValidationError({
                    'odometro_chegada': f"Odômetro de chegada ({self.odometro_chegada} km) não pode ser menor que o de saída ({self.odometro_saida} km)."
                })
